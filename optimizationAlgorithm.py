...
def __assignNewCenter(duration, clt, kTmpClt, df): 
    if len(kTmpClt[clt]) > 2:
        ...
        kTmpADO = df[df['Id'].isin(kTmpClt[clt])].groupby('Id')['ado'].apply(list) 
        newC = 1
...
def __kmeansFit(dbscanResult, k, duration, maxIter, adoArray, radius, ttlADOClt, inputArray):
    ...
    for i in range(5):
        ...
        originCenter_adj = list(originCenter) + list(center)
        maxIter = maxIter
        iter = 0
        ...
        while iter < maxIter:
            ...
            durationTmp = durationTmp[originCenter_adj,:]

            distance = durationTmp.min(axis=0)
            centerInd = durationTmp.argmin(axis=0)
            label = np.array(originCenter_adj)[centerInd]
            ...
            kTmpClt = df.groupby('center')['Id'].apply(list)
            newCenter = []
